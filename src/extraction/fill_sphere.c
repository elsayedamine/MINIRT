...
/*   Updated: 2025/08/15 16:38:21 by aelsayed         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include <miniRT.h>

int	auxilliary_fill_sphere(t_minirt *vars, t_object *obj, char **fields)
{
	int			type;
	t_object	data;
...
int	fill_sphere(t_minirt *vars, char *line, t_object *obj)
{
	...
	if (!(type >= RATIO))
		return (ft_free("2", fields), throw_error(SPHERE), 1);
	obj->r = data.r / 2.0f;
	return (auxilliary_fill_sphere(vars, obj, fields));
}
